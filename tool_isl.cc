...
int isl_printf_union_set(const char* format_template, __isl_keep isl_union_set* uset){
	int result = 0;

	result = isl_printf_str(format_template, isl_union_set_to_str(uset));

	return result;
}

int isl_printf_str(const char* format_template, __isl_take char* isl_str){
	int result = 0;

	result = printf(format_template, isl_str);
...
int isl_printf_union_map(const char* format_template, __isl_keep isl_union_map* umap){
	int result = 0;

	result = isl_printf_str(format_template, isl_union_map_to_str(umap));

	return result;
}

int isl_printf_schedule(const char* format_template, __isl_keep isl_schedule* sched){
	int result = 0;

	result = isl_printf_str(format_template, isl_schedule_to_str(sched));
...
__isl_give isl_set* convert_map_out_dims_to_set(__isl_keep isl_map *map){
...

	{
		...
		free(mapStr);
		mapStr = (char *)malloc(mapStrLen*sizeof(char));
