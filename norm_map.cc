...
static isl_stat statements_list_fill(__isl_take isl_map *map, void *user){
	...
	statementsList->statements = (statement_info *)realloc(statementsList->statements, statementsList->count*sizeof(statement_info));
	...
	statement->outDims = (out_dim_info*)malloc(statement->outDimCount*sizeof(out_dim_info));
		...
		for(int i=0; i<statement->outDimCount; i++){
			...
			if(strcmp(minVal, maxVal) == 0 || isl_set_get_dim_name(reverseDomain, isl_dim_set, i) == 0  ){
				if(isl_map_get_dim_name(reverseMap, isl_dim_in, i) != 0){
					statement->outDims[i].value = (char *)malloc((strlen(minVal)+1)*sizeof(char));
...
static void statement_add_order_dim(statement_info *statement, int dim){
	...
	statement->outDims = (out_dim_info *)realloc(statement->outDims, (statement->outDimCount+1)*sizeof(out_dim_info));
