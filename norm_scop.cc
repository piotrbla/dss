...
__isl_take loop_scop_list* normalize_calc_scop(__isl_keep loop_scop *orygScop){
	loop_scop_list *loopScopList = (loop_scop_list *)malloc(sizeof(loop_scop_list));
...
static isl_stat schedule_map_info_list_fill(__isl_take isl_map *map, void *user){
	...
	mapInfoList->scheduleMapInfoList = (schedule_map_info *)realloc(mapInfoList->scheduleMapInfoList, mapInfoList->scheduleMapInfoCount * sizeof(schedule_map_info));
...
static void schedule_map_info_list_sort(schedule_map_info_list *mapInfoList){
	...
	isl_bool *isGlobalOrderDim = (isl_bool*)malloc(outDimCount*sizeof(isl_bool));
...
static int  schedule_map_info_list_split(__isl_keep schedule_map_info_list **mapInfoList){
	...
	for(int i=0; i<oryginalMapInfoList->scheduleMapInfoCount; i++ ){
		...
		if(currentMapInfoList == 0){
			splitedScopes+=1;
			splitedMapInfoList = (schedule_map_info_list *)realloc(splitedMapInfoList, splitedScopes*sizeof(schedule_map_info_list));
		...
		currentMapInfoList->scheduleMapInfoList = (schedule_map_info *)realloc(currentMapInfoList->scheduleMapInfoList, currentMapInfoList->scheduleMapInfoCount * sizeof(schedule_map_info));
...
static void schedule_map_info_list_build_mapper(__isl_keep schedule_map_info_list *mapInfoList){
	dim_bounds *dimBounds = (dim_bounds *)malloc(mapInfoList->outDimCount * sizeof(dim_bounds));
...
static void schedule_map_info_list_set_global_Out_Dim_Info(__isl_keep schedule_map_info_list *mapInfoList){
	...
	mapInfoList->globalOutDimInfo = (global_out_dim_info *)malloc(mapInfoList->outDimCount * sizeof(global_out_dim_info));
...
static void schedule_map_info_init(schedule_map_info *mapInfo, __isl_keep loop_scop *loopScop, __isl_take isl_map *map){
	...
	mapInfo->outDimInfoList = (out_dim_info *)malloc(mapInfo->outDimCount*sizeof(out_dim_info));
...
isl_stat fill_bounds_from_constraint (__isl_take isl_constraint *constraint, void *user){
	...
	if(isl_constraint_is_upper_bound(constraint,isl_dim_set,dimBounds->dimIdx) == isl_bool_true){
		...
		dimBounds->upper.bounds = (char**) realloc(dimBounds->upper.bounds, sizeof(char*)*dimBounds->upper.counter);
		{
	...
	if(isl_constraint_is_lower_bound(constraint,isl_dim_set,dimBounds->dimIdx) == isl_bool_true){
		...
		dimBounds->lower.bounds = (char**)realloc(dimBounds->lower.bounds, sizeof(char*)*dimBounds->lower.counter);
