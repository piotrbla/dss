...
#include "loop_tile.hh"
#include "tool_isl.hh"
#include "tool_debug.hh"
#include "tool_string.hh"
...
__isl_give loop_tile_list* loop_tile_list_from_loop_scop_list(loop_scop_list *loopScopList){
	...
	loop_tile_list *loopTileList = (loop_tile_list*)malloc(sizeof(loop_tile_list));

	loopTileList->count = loopScopList->count;
	loopTileList->loopsTile = (loop_tile_info**)malloc(loopTileList->count*sizeof(loop_tile_info));
...
__isl_give loop_tile_info* loop_tile_from_loop_scop(__isl_keep loop_scop* loopScop){
	...
	do{
		...
		loopTile = (loop_tile_info*)malloc(sizeof(loop_tile_info));
...
static isl_stat loop_tile_read_dim_info_size(__isl_take isl_set *set, void *user){
	...
	loopTile->loopDimInfo = (loop_dim_info*)malloc(loopTile->loopDimSize*sizeof(loop_tile_info));
	...
	if(analyzeDim < loopTile->loopDimSize){
...

		{
			...
			if(lower_bound == isl_bool_false && upper_bound == isl_bool_false){
				...
				minValStr = (char *) malloc(2*sizeof(char));
				maxValStr = (char *) malloc(2*sizeof(char));
...
static void loop_tile_add_hyperplane(__isl_keep loop_tile_info* loopTile, __isl_keep isl_set *hyperplane){
	if(!loopTile->hyperplaneCheckInfo){
		loopTile->hyperplaneCheckInfo = (hyperplane_check_info*)malloc(sizeof(hyperplane_check_info));
...
static isl_stat loop_tile_space_from_hyperplanes(__isl_take isl_point *pnt, void *user){
	...
	loopTile->spaceInfo = (space_Info*)realloc(loopTile->spaceInfo, loopTile->hyperplaneDimSize * sizeof(space_Info));
	loopTile->spaceInfo[currentSpace].vector = (int*)malloc(loopTile->loopDimSize * sizeof(int));
...
static __isl_give isl_union_map * loop_tile_space_to_parallel_free_schedule(__isl_take isl_union_map *wafefrontScheduleMap, __isl_keep loop_tile_info* loopTile, const char *tupleName, char *mapToStr, id_name wfName){
	...
	for(int spaceInfoPos=0; spaceInfoPos<loopTile->hyperplaneDimSize; spaceInfoPos++){
		...
		if(spaceInfoPos < loopTile->daptParams->sizesDim){
			spaceSize = loopTile->daptParams->sizes[spaceInfoPos];
			spaceTimeSize =  loopTile->daptParams->sizesTime[spaceInfoPos];
		}

		sprintf(&tmpNamesStr[strlen(tmpNamesStr)],", b%s", spaceInfo->spaceName);
		sprintf(&tmpNamesStr[strlen(tmpNamesStr)],", t%s", spaceInfo->spaceName);
		...
		sprintf(&tmpEquationStr[strlen(tmpEquationStr)]," - b%s = 0", spaceInfo->spaceName);

		sprintf(&tmpEquationStr[strlen(tmpEquationStr)]," and %i * %s", spaceSize, spaceInfo->spaceName);
		...
		sprintf(&tmpEquationStr[strlen(tmpEquationStr)]," and %i * t%s", spaceTimeSize, spaceInfo->spaceName);
		sprintf(&tmpEquationStr[strlen(tmpEquationStr)]," <= b%s <= ", spaceInfo->spaceName);
		sprintf(&tmpEquationStr[strlen(tmpEquationStr)],"%i * (t%s + 1) - 1", spaceTimeSize, spaceInfo->spaceName);
	}

	sprintf(&equationStr[strlen(equationStr)],"] : exists %s : 1 = 1 %s", tmpNamesStr, tmpEquationStr );

	if(loopTile->wafefrontHyperplane!=0){
		sprintf(&equationStr[strlen(equationStr)]," and b%s = %i * t%s", wfName, get_pw_aff_value_to_int(isl_set_dim_min(isl_set_copy(loopTile->wafefrontHyperplane), 0)), loopTile->spaceInfo[0].spaceName);
		for(int spaceInfoPos=1; spaceInfoPos<loopTile->hyperplaneDimSize; spaceInfoPos++){
			sprintf(&equationStr[strlen(equationStr)]," + %i * t%s", get_pw_aff_value_to_int(isl_set_dim_min(isl_set_copy(loopTile->wafefrontHyperplane), spaceInfoPos)), loopTile->spaceInfo[spaceInfoPos].spaceName);
		}
