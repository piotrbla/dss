...
char* str_substring(const char* string, const char* start, const char* end){
	char* returnStr = NULL;
	char* tokenStart = (char *)strstr(string,start);
	char* tokenEnd = NULL;
	...
	if(tokenEnd != NULL && tokenStart != NULL){
		returnStr = (char *) malloc(sizeof(char)*((tokenEnd-tokenStart)+1));
...
char* str_concatenation(char* toExp, const char* toAdd){
	char* dest = (char *)malloc(sizeof(char)* (strlen(toExp) + strlen(toAdd) + 1));
